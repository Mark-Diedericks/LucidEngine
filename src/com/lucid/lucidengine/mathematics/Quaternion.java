...
public class Quaternion {
	...
	public Quaternion(Vector3f direction, Vector3f up) {
		this(new Matrix4f().initRotation(direction, up));
	}
	...
	public Quaternion mul(float value) {
		Vector3f vec3f = new Vector3f(value, value, value);
		return this.mul(vec3f);
	}
	
	public Quaternion add(Quaternion quat) {
		return new Quaternion(this.x + quat.getX(), this.y + quat.getY(), this.z + quat.getZ(), this.w + quat.getW());
	}
	
	public Quaternion add(float quat) {
		return new Quaternion(this.x + quat, this.y + quat, this.z + quat, this.w + quat);
	}
	...
	public Quaternion sub(float quat) {
		return new Quaternion(this.x - quat, this.y - quat, this.z - quat, this.w - quat);
	}
	
	public float dot(Quaternion quat) {
		return this.x * quat.getX() + this.y * quat.getY() + this.z * quat.getZ() + this.w * quat.getW();
	}
	...
	public Matrix4f toRotationMatrix() {
		return new Matrix4f().initRotation(getRotateForward(), getRotateUp(), getRotateRight());
	}
	
	public Vector3f getRotateForward() {
		return new Vector3f(2.0f * (x*z - w*y), 2.0f * (y*z + w*x), 1.0f - 2.0f * (x*x + y*y));
	}
	
	public Vector3f getRotateUp() {
		return new Vector3f(2.0f * (x*y + w*z), 1.0f - 2.0f * (x*x + z*z), 2.0f * (y*z - w*x));
	}
	
	public Vector3f getRotateRight() {
	...
	public Quaternion set(Quaternion quat) {
		return this.setQuaternion(quat.getX(), quat.getY(), quat.getZ(), quat.getW());
	}
	
	public Quaternion setQuaternion(float x, float y, float z, float w) {
		this.x = x;
		this.y = y;
		this.z = z;
		this.w = w;
	...
	public Vector3f getXZRotateForward() {
		Vector3f xz = this.getRotateForward();
		xz.setY(0);
		return xz;
	}
}
