...
public class GuiCheckBox {
	...
	public GuiCheckBox(GuiEngine guiEngine, Texture texture, int x, int y, int width, int height, boolean isEnabled) {
	...
	public GuiCheckBox(GuiEngine guiEngine, String texture, int x, int y, int width, int height, boolean isEnabled) {
		this(guiEngine, new Texture(texture), x, y, width, height, isEnabled);
	}
	...
	public Button isClicked(int button, int x, int y) {
		...
		if(x > this.x*scale && y < (EngineWindow.height - this.y*scale) && x < (this.x + this.width)*scale && y > (EngineWindow.height - (this.y + this.height)*scale)) {
			if(Button.valueOf(button) == Button.LEFT) this.isEnabled = !this.isEnabled;
